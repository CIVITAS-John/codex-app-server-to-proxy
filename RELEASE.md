...
- [ ] The publish job pins npm `11.18.0` instead of trusting the npm version bundled with the moving Node.js 24 runner image. Before changing a remote ref, it requests an npm-audience GitHub OIDC token, verifies the repository, workflow, `main` ref, `npm` environment, and GitHub-hosted runner claims, and confirms npm accepts that identity. It logs or persists neither the OIDC token nor the exchanged temporary publish credential.
