...
Stages 01 through 06 are complete. The deterministic offline gate covers app-server ownership, Chat Completions translation, dynamic-tool lifecycles, continuation validation, restart recovery, atomic state migration, per-request policy enforcement, and failure paths. The focused `npm run test:live` contract passed on 2026-07-14 with two scenarios under its four-`gpt-5.4-mini`-turn guard: streaming role history, then a function-tool round trip followed by completed-thread continuation after restarting both the proxy and app-server. The tool round trip itself stayed within its two-call guard. Stage 06 added exact cwd, sandbox, approval, managed-requirement, and web-search mappings without making live model calls; per-thread web-search behavior therefore remains eligible for a later opt-in smoke test. Stage 08 owns packed-tarball installation and bin-shim proof.
