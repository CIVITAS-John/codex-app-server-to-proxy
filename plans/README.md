...
- Depend on exact `@openai/codex 0.155.1` for default executable resolution and the generated contract. An explicit `--codex-path` override must report that same version; older and newer executables are rejected until their contracts are reviewed.
- Run the package-owned app-server in the proxy-owned `~/.codex-openai-proxy/codex-home` by default, shared across roots but isolated from the ordinary Codex CLI home. On every startup, `--sync-auth always` adopts `auth.json` from the pre-existing Codex home only when the target is missing or the source is strictly newer; `--sync-auth never` keeps a proxy-only login isolated. These are the only synchronization modes. Allow `--codex-home` to select another directory.
    ...
    - The current unversioned proxy home is reviewed for the `0.155.1` contract. The 0.154.0 to 0.155.1 upgrade adds only unused generated app-server fields/methods, leaves proxy-owned persisted filenames and schemas unchanged, and preserves model-cache metadata. The Responses Lite workaround remains pending live proof for removal. A future Codex pin must repeat that review or adopt an explicit versioned-home or migration decision before release.
...
- Use mocks by default and only `gpt-6-luna` for opt-in live development tests.
...
- The expanded live contract was attempted on 2026-09-22 under its hard maximum of 32 deduplicated `gpt-6-luna` upstream model responses. Setup stopped because no available Codex model cache contained metadata for the selected model; 0 provider responses were used, so compatibility and normal-count calibration remain pending. Earlier repository notes describe a 2026-07-14 two-scenario run under a prior four-call guard, but they do not record an exact count, commit, or workflow URL and are not Stage 08 release evidence.
