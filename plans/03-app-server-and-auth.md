...
5. Query account state at startup. If unauthenticated, call `account/login/start` with ChatGPT browser login.
    ...
    - As a temporary Codex `0.145.0` compatibility patch, clone that home's `models_cache.json` into a separate catalog, set `use_responses_lite` to `false` for every model, and select the clone with `model_catalog_json` before app-server becomes ready. Never edit the refreshable cache directly.
...
- Model-catalog tests prove the source cache remains unchanged, every cloned model disables Responses Lite, unrelated Codex config survives, reruns are idempotent, and a cache created during setup causes exactly one private restart.
...
The pinned Codex `0.145.0` startup now installs a temporary regular-Responses model catalog in the selected Codex home. The proxy atomically derives `models.no-responses-lite.json` from `models_cache.json`, preserves unknown catalog metadata, forces `use_responses_lite: false` on every model, and manages a marked top-level `model_catalog_json` block without replacing unrelated `config.toml` content. A cache that appears only after first-run initialization triggers one private app-server restart before readiness. The compatibility consequence is that the selected catalog is frozen at the cached metadata and any previous top-level catalog override is replaced while the workaround is installed; the Codex-owned cache remains untouched and refreshable for later removal of the patch. The opt-in live backend uses an ephemeral proxy-style Codex home, may force a zero-model-call `model/list` refresh to obtain the first cache, and fails before any turn unless the replacement process loaded the override. Its dynamic-tool scenario then requires two independent calls in one batch, making parallel dispatch the live behavioral check for the workaround.
