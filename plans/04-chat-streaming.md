...
- `rawResponse/completed` carries exact per-upstream-response usage. That nullable per-request delta remains excluded from HTTP usage attribution because the cumulative `thread/tokenUsage/updated` subtraction contract already conserves usage without guessing whether the same tokens reappear in a later `total` or mapping `cacheWriteInputTokens`. The event itself is now consumed as control flow: with `thread/start.experimentalRawEvents: true`, app-server emits it once after each upstream Responses completion, making it the authoritative close of a dynamic-tool callback batch. `rawResponseItem/completed` and the raw usage payload remain unexposed. Recovery is strictly additive: after the turn has completed, an aborted request, a lost transport, or ingress overflow ends usage collection and the response still reports its terminal frame and records its continuation mapping.
