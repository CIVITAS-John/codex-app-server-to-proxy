...
- Usage attribution begins only after `turn/start` returns and accepts only matching thread and turn notifications. One response can span several app-server model requests, and `tokenUsage.last` describes only the most recent one, so the response reports the exact cumulative `tokenUsage.total` counters gained since the turn's pre-turn baseline, derived as `total - last` on the first attributed notification. Repeating one breakdown therefore cannot double count it. When either breakdown omits a counter, or the cumulative counters drop because app-server reset them, the exact `tokenUsage.last` values are mapped instead; unavailable details are omitted and never synthesized.
- Usage is emitted after the terminal frame, including for a client tool suspension, and a `thread/tokenUsage/updated` notification that arrives with or after `turn/completed` is still consumed. Recovery never waits beyond frames the transport has already read, so a turn whose usage never arrives still completes without it.
