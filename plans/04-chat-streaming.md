...
- Usage is emitted after the terminal frame, including for a client tool suspension. A completed turn keeps consuming correlated notifications through the matching `thread/status/changed: idle` lifecycle boundary, so usage delivered on a later transport read after `turn/completed` is retained without guessing event-loop timing. A client-tool suspension has no idle transition and stops at the first correlated usage, and skips the wait entirely when its suspended batch already carried usage, so the common ordering costs no added latency per tool round trip. Both paths share one configurable cap (`--usage-grace`, default 12 s, `0` disables; the wait ends at the first correlated usage — live app-server flushes a parked turn’s usage about ten seconds after it issues the dynamic tool call, so the default clears that flush with margin), so a boundary that never arrives costs a bounded wait and omitted usage rather than a stalled response.
- Attribution is conserved across a suspension: every response persists the exact boundary its successor subtracts from, which is the newest total it reported or the boundary it started from when app-server attributed nothing to it. A suspended response that reports no usage therefore hands its unreported model requests to its continuation instead of stranding them, and the union of the two responses counts every token exactly once. The compatibility consequence is that whether app-server emits usage before `item/tool/call` only decides *which* response reports those tokens, never whether they are reported. A `total` without an attributable `last` never advances the boundary, since no response emitted its delta.
- `rawResponse/completed` carries exact per-upstream-response usage and was evaluated as a source for the suspension window. It is rejected and classified diagnostic-only: it is a per-request delta rather than a cumulative total, so folding it into the subtraction model would require inferring whether the same tokens reappear in a later `total`; it carries `cacheWriteInputTokens`, which the counters do not model; it is nullable; it is undocumented in the pinned protocol reference; and its ordering relative to the tool dispatch is exactly as unverified as `thread/tokenUsage/updated`'s. Revisit only once it is documented upstream, a live spike proves both its ordering and whether its tokens reappear in a later cumulative total, and `cacheWriteInputTokens` has a decided mapping. After boundary conservation it would buy earlier reporting, not correctness. Recovery is strictly additive: after the turn has completed, an aborted request, a lost transport, or ingress overflow ends usage collection and the response still reports its terminal frame and records its continuation mapping.
