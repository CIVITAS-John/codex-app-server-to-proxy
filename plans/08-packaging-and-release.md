...
- The opt-in live smoke must be explicitly authorized, use only `gpt-5.6-luna`, state the expected normal count of eleven and hard maximum of twelve model calls on POSIX before execution, and record the exact count afterward. Its client-defined tool scenario must reach the third consecutive full-history tool-result request.
