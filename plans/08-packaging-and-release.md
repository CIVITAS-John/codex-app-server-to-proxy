...
- The package-owned app-server runs with the configured isolated Codex home. Best-effort newest-wins auth synchronization logs only a sanitized error code on failure, so an arbitrary source `CODEX_HOME` cannot enter structured logs. An RPC-reported unusable login follows one bounded logout and browser/device-code recovery attempt before startup fails closed.
