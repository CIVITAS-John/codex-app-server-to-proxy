...
- Every candidate after the name-reserving bootstrap uses a `main`-only manual GitHub Actions dispatch with npm trusted publishing. The workflow requires the full remote matrix, creates the prerelease version commit and tag on its runner, tests and publishes the exact retained tarball with provenance, and then fast-forwards `main`. Rebuilding at `npm publish` time is forbidden. This replaces local versioning and tag-triggered publication; failures before the tag push leave the repository untouched, while an unsuccessful publish command deletes the remote tag only when the tag still exists and npm returns E404 for the exact version. An existing version or unverifiable registry retains the tag; because a leftover tag rejects the next run's tag push, operators recheck npm and delete a tag retained for a confirmed-unpublished version before redispatching. A post-publish `main` failure requires manually merging the tagged commit into `main`. GitHub provenance identifies the dispatch commit rather than its runner-created child, so release evidence records both commits and verifies their parent/tag relationship.
...
- The checked-in prerelease workflow rejects non-`main` dispatches, first requires the full reusable offline matrix, and confirms the package already exists for OIDC. It creates the RC version commit and tag locally, rejects stable or already-published versions, reruns the offline release gate on the bumped tree, runs the packed smoke with `--retain`, and uploads the exact tested tarball for 14-day evidence retention. A dry run stops before remote changes. A real run pushes only the tag, publishes that tarball to `next` through the npm environment with provenance, and fast-forwards `main`; after a failed publish command it confirms the tag exists, retains it for an existing or unverifiable npm version, and deletes it on E404 so the next dispatch can recreate it.
