...
- The exact `@openai/codex 0.154.0` runtime dependency owns default executable resolution and the generated app-server contract. An explicit `--codex-path` override must report the same version. Older and newer Codex executables are rejected until their generated contracts are reviewed and checked in.
...
- The unversioned proxy home is accepted for the reviewed `0.154.0` contract. The `0.153.4` to `0.154.0` upgrade leaves proxy-owned persisted filenames and schemas unchanged, preserves unknown model-catalog metadata, and changes only additive generated app-server fields outside the public proxy surface, so no home migration is required. The Responses Lite workaround remains installed because the upstream release notes do not establish that its direct parallel-tool framing issue is fixed; an authorized live contract is still required before removal. A future Codex upgrade must repeat this compatibility review or ship an explicit versioned-home or migration decision before publication.
    - **Review (2026-09-14):** [Pinned 0.154.0 source](../docs/codex-app-server.md#pinned-compatibility-note) still uses `auth.json`, `models_cache.json`, and `model_catalog_json`; the separate override preserves old cache version/ETag metadata. No proxy home/store migration is needed. New application-network requirements remain app-server-enforced, and verification requests are declined. Offline package smoke, the normal pull-request OS matrix, and separately authorized live verification retain distinct evidence requirements.
