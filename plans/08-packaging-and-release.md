...
- The exact `@openai/codex 0.153.4` runtime dependency owns default executable resolution and the generated app-server contract. An explicit `--codex-path` override must report the same version. Older and newer Codex executables are rejected until their generated contracts are reviewed and checked in.
