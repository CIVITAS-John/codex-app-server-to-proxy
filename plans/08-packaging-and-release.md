...
- The exact `@openai/codex 0.155.1` runtime dependency owns default executable resolution and the generated app-server contract. An explicit `--codex-path` override must report the same version. Older and newer Codex executables are rejected until their generated contracts are reviewed and checked in.
...
- The unversioned proxy home is accepted for the reviewed `0.155.1` contract. The 0.154.0 to 0.155.1 upgrade adds generated app-server fields and methods outside the proxy surface, leaves proxy-owned persisted filenames and schemas unchanged, and preserves model-cache metadata, so no home migration is required. The Responses Lite workaround remains installed because the pinned client still uses prompt-only tool schemas and disables parallel calls for those model entries; an authorized live contract is still required before removal. A future Codex upgrade must repeat this compatibility review or ship an explicit versioned-home or migration decision before publication.
    - **Review (2026-09-22):** [Pinned 0.155.1 source](../docs/codex-app-server.md#pinned-compatibility-note) still uses `auth.json`, `models_cache.json`, and `model_catalog_json`; the separate override preserves old cache version/ETag metadata. No proxy home/store migration is needed. New application-network requirements remain app-server-enforced, and verification requests are declined. Offline package smoke, the normal pull-request OS matrix, and separately authorized live verification retain distinct evidence requirements.
...
| Opt-in `gpt-6-luna` live smoke with exact provider-response record | Targeted spawn scenario passed on 2026-09-22; full suite and exact normal count pending |
