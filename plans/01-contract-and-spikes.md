...
- Usage semantics:
    ...
    - `rawResponse/completed` is internal-only and carries a per-upstream-response usage delta. Its usage remains excluded from HTTP attribution, but its documented once-per-upstream-completion ordering is now the authoritative dynamic-tool batch boundary. See `plans/04-chat-streaming.md` and `plans/05-tools-and-threads.md`.
