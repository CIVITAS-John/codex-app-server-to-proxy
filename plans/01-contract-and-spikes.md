...
- Usage semantics:
    ...
    - `rawResponseItem/completed` direct declared calls and the once-per-upstream `rawResponse/completed` event now define dynamic-tool membership and its exact boundary. Their provider-native payloads and the completion's usage delta remain excluded from HTTP output; only declared calls are normalized to standard `tool_calls`. See `plans/04-chat-streaming.md` and `plans/05-tools-and-threads.md`.
...
- Generated experimental TypeScript and JSON Schema artifacts from the exact package-owned Codex version; commands and executable-source decision are recorded in `protocol/VERSION.json`. The contract was refreshed to `codex-cli 0.146.0` on 2026-07-30.
