...
- Ready and pending-tool records retain the latest complete cumulative app-server `usageTotal` observed at their response boundary. The next response uses it as the only authoritative subtraction baseline. Pre-upgrade schema-version-0 records without this optional field fall back to exact `tokenUsage.last` for one response instead of fabricating an earlier total.
