...
- Two abandonment cases deliberately keep that fallback rather than adding a supersede protocol: a client that disconnects immediately after the tool-call frame, and a continuation that races in before the suspended response finishes. Both leave the record without a boundary, so the continuation maps exact `last` — the pre-fix behavior, never a double count. Hardening would mean writing the boundary synchronously in `suspend()` plus suppressing the suspended response's usage frame once a continuation has consumed it.
...
- One response store lives for the proxy server lifetime. Replacing or removing app-server disposes the old continuation generation, cancels deadlines, rejects suspended responders and late callbacks, expires their mappings, and then closes the old transport so active executions wake without writing stale mappings. Persistence attempted from timeout and disposal callbacks is best-effort after in-memory ownership is released: synchronous disk failure cannot escape those lifecycle callbacks, and a missed pending-to-expired write self-heals during the next store load. Buffered frames are ignored after logical transport close.
