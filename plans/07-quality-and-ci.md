...
4. Expand the opt-in live contract without duplicating the offline matrix.
    ...
    - The client-defined tool scenario replays the complete transcript through three consecutive `role: "tool"` result requests, then resumes the completed response after restart. Together with the other scenarios, keep the expected normal total at eleven `gpt-5.6-luna` calls and set the suite-wide hard guard to twelve so the authorized run retains one call of retry headroom.
...
- Opt-in live tests run only through the explicitly selected live configuration, hard-code `gpt-5.6-luna`, declare eleven normal POSIX calls with a twelve-call maximum, and cover role-history SSE, three consecutive full-history client dynamic-tool result continuations, restart continuation, default disabled execution, and, on POSIX hosts, explicit safe policy selection, built-in tool streaming, an output-dependent continuation, and fresh replay with internal activity stripped.
