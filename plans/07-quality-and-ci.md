...
4. Expand the opt-in live contract without duplicating the offline matrix.
    ...
    - App-server may display that command directly as `pwd` or behind a known POSIX shell launcher with exact `-lc` arguments and an optionally single- or double-quoted `pwd` payload. Compatibility accepts only that bounded wrapper; compound commands, arbitrary shell paths, and additional arguments fail the live assertion even if they contain `pwd`.
...
The safe-policy live smoke treats app-server's built-in command string as a display contract rather than requiring byte-for-byte `pwd`: it accepts direct `pwd` and a tightly allowlisted POSIX shell `-lc` wrapper whose entire payload is `pwd`, optionally with simple single or double quotes. The compatibility consequence is deliberate rejection of compound commands, extra shell arguments, unrecognized launchers, and arbitrary paths.
