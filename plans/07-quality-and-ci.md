...
4. Expand the opt-in live contract without duplicating the offline matrix.
    ...
    - The built-in-tool turn, its persisted continuation, and its fresh replay permit at most three additional model calls. Raise the suite-wide hard guard to seven `gpt-5.4-mini` calls and document the expected normal total before running it.
