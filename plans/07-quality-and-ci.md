...
4. Expand the opt-in live contract without duplicating the offline matrix.
    ...
    - Add one serial `gpt-5.4-mini` scenario using that explicit safe `x_codex` policy tuple and the ephemeral test root. Pre-seed a fixture with a random token, requesting mode `0600` on POSIX, require one bounded read-only built-in command to read it, then assert streamed observational `tool_calls` and correlated `tool_results`, including a terminal result with completed status, exit code zero, and content equal to the token. The built-in result is already executed Codex activity: it must not produce `finish_reason: "tool_calls"` or be sent back as a client `role: "tool"` message.
    ...
    - Add one default-policy `gpt-5.4-mini` scenario that asks to run the bounded observation command and proves the `disabled` sandbox completes as pure chat: no built-in tool calls or results are exposed, and the observation token is absent.
    ...
    - The built-in-tool turn, its persisted continuation, and its fresh replay permit at most three additional model calls. Keep the expected normal total at eight `gpt-5.4-mini` calls and set the suite-wide hard guard to nine so the authorized run retains one call of retry headroom.
...
- Opt-in live tests run only through the explicitly selected live configuration, hard-code `gpt-5.4-mini`, declare eight normal POSIX calls with a nine-call maximum, and cover role-history SSE, a client-defined dynamic-tool round trip, restart continuation, default disabled execution, and, on POSIX hosts, explicit safe policy selection, built-in tool streaming, an output-dependent continuation, and fresh replay with internal activity stripped.
