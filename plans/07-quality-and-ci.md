...
4. Expand the opt-in live contract without duplicating the offline matrix.
    ...
    - The built-in-tool turn, its persisted continuation, and its fresh replay permit at most three additional model calls. Keep the expected normal total at eight `gpt-5.6-luna` calls and set the suite-wide hard guard to nine so the authorized run retains one call of retry headroom.
