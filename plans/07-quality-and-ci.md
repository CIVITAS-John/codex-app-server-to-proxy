...
4. Expand the opt-in live contract without duplicating the offline matrix.
    ...
    - Define high reasoning effort, `read-only`, and disabled web search as live prerequisites. If the selected model or managed requirements disallow a value, report the unsupported prerequisite before starting model work rather than weakening policy or substituting a different mode. A successful Stage 07 live verification record must include these scenarios passing.
    - Add one serial `gpt-5.6-luna` scenario using that explicit safe `x_codex` policy tuple and the ephemeral test root. Pre-seed a fixture with a random token, requesting mode `0600` on POSIX, require one bounded read-only built-in command to read it, then assert streamed observational `tool_calls` and correlated `tool_results`, including a terminal result with completed status, exit code zero, and content equal to the token. The built-in result is already executed Codex activity: it must not produce `finish_reason: "tool_calls"` or be sent back as a client `role: "tool"` message.
    - App-server may display that command directly as `cat .codex-contract-observation` or behind a known POSIX shell launcher with exact `-lc` arguments and an optionally single- or double-quoted complete payload. Compatibility accepts only that bounded wrapper; compound commands, arbitrary shell paths, and additional arguments fail the live assertion even if they contain the fixture name. This particular live fixture is scoped to POSIX hosts and does not claim PowerShell compatibility; Windows live registration omits the scenario before backend startup, while its deterministic fake remains cross-platform.
    - Continue the completed response with its nonstandard `previous_response_id`, a new user message, and the identical `x_codex` settings. Verify that `thread/resume` reuses the same Codex thread and that the model returns the unpredictable prior output token without running another tool. The prompt does not reveal that token.
    ...
    - The client-defined tool scenario first requires two independent calls in one parallel batch, replays the complete transcript through three consecutive `role: "tool"` result requests, then resumes the completed response after restart. Together with the other scenarios, keep the expected normal total at eleven `gpt-5.6-luna` calls and set the suite-wide hard guard to twelve so the authorized run retains one call of retry headroom.
    - The built-in-tool turn, its persisted continuation, and its fresh replay permit at most three additional model calls.
...
- Opt-in live tests run only through the explicitly selected live configuration, hard-code `gpt-5.6-luna`, declare eleven normal POSIX calls with a twelve-call maximum, and cover role-history SSE, a required two-call parallel client-tool batch, three consecutive full-history client dynamic-tool result continuations, restart continuation, default disabled execution, and, on POSIX hosts, explicit safe policy selection, built-in tool streaming, an output-dependent continuation, and fresh replay with internal activity stripped. Live setup uses an ephemeral proxy-style Codex home and refuses to make a model call unless the temporary regular-Responses catalog override loaded successfully.
...
The safe-policy live smoke treats app-server's built-in command string as a POSIX display contract: it accepts direct `cat .codex-contract-observation` and a tightly allowlisted POSIX shell `-lc` wrapper whose entire payload is that command, optionally with simple single or double quotes. This particular live fixture is scoped to POSIX hosts and does not claim a PowerShell spelling; Windows live registration omits it before backend startup, while the deterministic fake contract remains cross-platform. The fixture contains a per-run random token unknown to the continuation prompt. The smoke validates the correlated terminal result structurally and against that token, checks that first-turn assistant prose does not disclose the token, resumes the same thread, and requires the model to return the prior stdout without another tool call. The compatibility consequence is deliberate rejection of compound commands, extra shell arguments, unrecognized launchers, and arbitrary paths while avoiding cwd representation and nondeterministic prose as compatibility claims.
