...
4. Expand the opt-in live contract without duplicating the offline matrix.
    ...
    - The built-in-tool turn, its persisted continuation, and its fresh replay permit at most three additional model calls. Keep the expected normal total at seven `gpt-5.4-mini` calls and set the suite-wide hard guard to eight so the authorized run retains one call of retry headroom.
...
- Opt-in live tests run only through the explicitly selected live configuration, hard-code `gpt-5.4-mini`, declare seven normal POSIX calls with an eight-call maximum, and cover role-history SSE, a client-defined dynamic-tool round trip, restart continuation, and, on POSIX hosts, explicit safe policy selection, built-in tool streaming, an output-dependent continuation, and fresh replay with internal activity stripped.
