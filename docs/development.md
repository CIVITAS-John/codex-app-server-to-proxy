...
The dedicated system-prompt contract checks aggregate and SSE responses to a system-only random nonce with conflicting user input. It disables execution environments, web search, and subagents, uses only `gpt-5.6-luna`, and has its own two-response ceiling:
