...
The offline Vitest setup mocks the idle-usage grace to zero so ordinary requests do not spend five seconds waiting. Delayed-wire integration cases opt into a short local window. Clock-driven execution tests load the production duration and advance fake timers to verify late corrections, repeated idle notifications, the terminal backstop, and aborts. The live configuration and built CLI retain the production timing; do not raise offline test timeouts to accommodate the grace period.
