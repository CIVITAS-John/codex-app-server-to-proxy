...
`rawResponse/completed` also supplies exact fallback usage, summed once per upstream response ID. A later attributable thread update replaces the raw sum; the sources are never added together. A replayed raw response cannot undo that correction. If any observed raw completion lacks valid base counters, its turn-wide raw sum is unavailable and the proxy uses available thread usage instead. Cached and reasoning details require those counters in every summed response. Only normalized counts enter HTTP output; raw provider metadata stays private.

Completed and interrupted turns collect updates for a fixed five-second window after the first idle notification. Earlier usage and later corrections do not end or extend that window. A ten-second terminal collection backstop, request abort, or transport/queue failure can end it sooner. This adds up to five seconds after idle before aggregate output or streaming terminal frames; text and tool deltas still stream as they arrive.
...
Each successful response records the exact boundary the next response should use. When raw usage is selected, complete counters advance the starting baseline by exactly the reported sum; later cumulative accounting therefore cannot charge it twice. A late complete thread snapshot replaces that boundary. If selected raw usage lacks details or the starting baseline is unknown or invalidated by a counter reset, no cumulative boundary is saved, and the continuation uses attributable `last` usage. The stored shape remains unchanged. A pending tool response that receives no usage keeps its starting boundary, allowing the continuation to report those tokens when app-server later attributes them. See [`src/http/chat-normalize.ts`](../src/http/chat-normalize.ts) for attribution and [`src/http/chat-execute.ts`](../src/http/chat-execute.ts) for persistence at terminal events.

Final `usage_unreported` warnings carry `missing: "all"` when no usage exists or `missing: "reasoning"` when base counters exist without reasoning. A zero reasoning count is present. `usage_source` identifies `raw_response`, `thread_token_usage`, or `none`. Debug events `usage_notification`, `usage_tool_interrupt`, `usage_thread_idle`, and `usage_collection_completed` record elapsed milliseconds, correlation checks, counter presence, and notification counts without logging provider payloads or token values. Collection still runs when HTTP usage output is disabled.
